...
class UsersController < ApplicationController
  ...
  def fetch_readable_users(message)
    ...
    UserListFetcher.fetch_all(message, current_user.readable_users_for_current_user(admin_roles))
  end

  def fetch_user_if_readable(desired_guid)
    readable_users = current_user.readable_users_for_current_user(permission_queryer.can_read_globally?)
