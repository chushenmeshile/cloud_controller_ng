    ...
    describe '.readable_users_for_current_user' do
      ...
      context 'when an {admin, read_only_admin, global_auditor} lists users' do
        it 'sees all the org users in managed org' do
          expect(User.make(guid: 'global-user').readable_users_for_current_user(true).map(&:guid)).
            to(match_array([
              'global-user',
              'org_1_manager',
      ...
      shared_examples 'an org_user' do
        it 'can view all other users in their org' do
          expect(role.readable_users_for_current_user(false).map(&:guid)).
            to(match_array(%w(
              org_1_manager
              org_1_billing_manager
              org_1_auditor
      ...
      context 'in the 2nd org' do
        it 'can view the users in their org but not in the first' do
          expect(space_2a_manager.readable_users_for_current_user(false).map(&:guid)).
            to(match_array([
              'space_2a_manager',
              'space_2a_auditor',
              'space_2a_developer',
